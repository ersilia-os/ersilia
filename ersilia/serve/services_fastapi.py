...
class BaseServing(ErsiliaBase):
    def __init__(self, model_id, config_json=None, preferred_port=None, url=None):
        ...
        self.bundle_path = self._get_bundle_location(model_id=self.model_id)
        self._preferred_port = preferred_port
        self._url = url

    def _get_info_from_bundle(self):
        info_file = os.path.join(self.bundle_path, "info.json")
        with open(info_file, "r") as f:
            info = json.load(f)
        return info

    def _get_apis_from_bento(self):
        info = self._get_info_from_bundle()
        return info["apis"]

    def _get_info_from_url(self):
        response = requests.get("{0}/info".format(self.url))
        return response.json()
    
    def _get_apis_from_url(self):
...
class _FastAPIService(BaseServing):
    ...
    def _fastapi_serve(self, runcommand_func=None):
        ...
        sl += [
            "ersilia_model_serve --bundle_path {0} --port {1} &> {2} &".format(
                self.bundle_path, self.port, tmp_file
            )
        ]
        sl += ["_pid=$!"]
...
class SystemBundleService(_FastAPIService):
    def __init__(self, model_id, config_json=None, preferred_port=None, url=None):
        _FastAPIService.__init__(
            self,
            model_id=model_id,
            config_json=config_json,
            preferred_port=preferred_port,
...
class CondaEnvironmentService(_FastAPIService):
    def __init__(self, model_id, config_json=None, preferred_port=None, url=None):
        _FastAPIService.__init__(
            self,
            model_id=model_id,
    ...
    def serve(self):
        self._fastapi_serve(self._run_command)
