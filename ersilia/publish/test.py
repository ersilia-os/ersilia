...
class ModelTester(ErsiliaBase):
    ...
    """
    This function uses the scikit vectorizer package to compare the differences between outputs when 
    they're strings and not floats
    """
    def _compare_output_strings(self, output1, output2): 
        ratio = fuzz.ratio(output1, output2)
        return ratio
    ...
    def make_output(self, output, time):
        ...
        data = {"model size": {"KB": size_kb, "MB": size_mb, "GB": size_gb}, 
                "time to run model (seconds)": time, 
...
# To do: 
# 1. test it with normal run and then try the bash run.sh, comparing the two outputs 
# 2. Record CPU and memory usage (computational resources)

# scikit vectorizer: use this to measure distance between two strings 
