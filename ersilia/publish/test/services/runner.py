...
class RunnerService:
    ...
    def run_model(self, inputs: list, output: str, batch: int):
        ...
        cmd = f"ersilia serve {self.model_id} --cache-saving disabled --cache-retrieving disabled && ersilia run -i '{inputs}' -o {output} -b {str(batch)}"
        out = run_command(cmd)
        return out
    ...
    def run_bash(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                cmd = f"ersilia serve {self.model_id} --cache-saving disabled --cache-retrieving disabled && ersilia -v run -i {os.path.abspath(input_file_path)} -o {output_path}"
