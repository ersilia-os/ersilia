...
class ApiErsiliaError(ErsiliaError):
    def __init__(self):
        self.message = "Error occurred while running api command"
        self.hints = ""
...
class InputFileNotFoundError(ErsiliaError):
    def __init__(self, file_name):
        ...
        self.hints = "Check the path. Accepted formats are .csv, .tsv and .json."
        ErsiliaError.__init__(self, self.message, self.hints)


class UnprocessableInputError(ErsiliaError):
    def __init__(self):
        self.message = "The input could not be processed, so no output was written."
        self.hints = "Check that the input is valid for this model (e.g. valid SMILES)."
