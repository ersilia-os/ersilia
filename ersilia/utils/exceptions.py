...
class InvalidModelIdentifierError(ErsiliaError):
    ...
    def __init__(self, model):
        self.model = model
        self.message = "Could not identifiy model identifier or slug {0}:".format(self.model)
        self.hints = "Please check that {0} exists in the Ersilia Model Hub:\n - https://ersilia.io/model-hub (for approved models)\n - https://airtable.com/shrUcrUnd7jB9ChZV (for approved and in preparation models)".format(self.model)
        super().__init__(self.message, self.hints)
...
class ModelNotAvailableLocallyError(ErsiliaError):
    ...
    def __init__(self, model):
        self.model = model
        self.message = "Model {0} is not available locally, so it cannot be served".format(self.model)
...
class EmptyOutputError(ErsiliaError):
    ...
    def __init__(self, model_id, api_name):
        ...
        self.message = "Model API {0}:{1} did not produce an output".format(self.model_id, self.api_name)
