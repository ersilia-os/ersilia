...
6. Start coding! Head to the [Model Incorporation Guidelines](./#anatomy-of-the-ersilia-model-template) for more information.
...
{% hint style="success" %}
A list of `eos` identifiers is provided in the [Ersilia Model Hub Spreadsheet](https://docs.google.com/spreadsheets/d/1TQdei8kkF6zMGyDn0km0qmjZb6p-PM9gsBnSWg3637s/edit?usp=drive\_web\&ouid=114775674178390159004). You can read about this Spreadsheet [here](broken-reference).
{% endhint %}
...
Now we already have the input file of the `run_predict.sh`script, located in the `model/framework/` directory, as specified [above](./#the-model-folder). The following creates a dummy Bash script in the temporary directory and runs the command from there. The output is saved in the temporary directory too. [Remember](./#the-model-folder) that the `run_predict.sh` script expects three arguments, `FRAMEWORK_DIR`_,_ `DATA_FILE` and `OUTPUT_FILE`.
...
Now it is time to write some code. Here we will follow the description of the `model` folder [given above](./#the-model-folder).
...
By default, for chemical compound inputs, Ersilia uses single-column files with a header (see the `service.py` file [above](./#the-service-file)). However, the `sascorer.py` [expects](./#test-the-model) a two-column file. Let's write an **input** adapter:
...
So far, we haven't pointed to the model parameters. When [migrating code and parameters](./#migrate-code-and-parameters), we separated the `sascore.py` file and the `fpscores.pkl.gz` file.
...
Don't forget document the model. Read the [instructions to write the `README` file](./#the-readme-file) page. Feel free to ask for help in the Slack `#internships` channel.
