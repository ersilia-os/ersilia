...
@pytest.mark.parametrize("smiles, expected", [("C", True), ("CCO", True)])
...
@pytest.mark.parametrize("smiles, expected", [("invalid_smiles", False), ("", False)])
...
@pytest.mark.parametrize(
    "inchikey, expected",
    [("invalid_inchikey", False), ("BQJCRHHNABKAKU-KBQPJGBKSA", False)],
)
...
def test_guess_type_inchikey(compound_identifier, inchikey, expected):
    ...
    assert (
        result == expected
    ), f"Expected 'inchikey', but got '{result}' for input '{inchikey}'"
...
def test_guess_type_smiles(compound_identifier, smiles, expected):
    ...
    assert (
        result == expected
    ), f"Expected 'smiles', but got '{result}' for input '{smiles}'"
...
def test_guess_type_whitespace(compound_identifier, whitespace_input, expected):
    ...
    assert (
        result == expected
    ), f"Expected '{UNPROCESSABLE_INPUT}' for input '{whitespace_input}'"


@pytest.mark.parametrize(
    "non_char_input, expected",
    [
